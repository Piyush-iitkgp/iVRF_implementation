...
void eval(unsigned char *v, unsigned char *y, TREE_NODE *ap, unsigned char *pk,
  ...
  falcon_keygen_make(&sc_key, FALCON_LOGN, sk, FALCON_PRIVKEY_SIZE(FALCON_LOGN),
                     pk, FALCON_PUBKEY_SIZE(FALCON_LOGN), buf,
                     FALCON_TMPSIZE_KEYGEN(FALCON_LOGN));
  ...
  *sig_len = FALCON_SIG_COMPRESSED_MAXSIZE(FALCON_LOGN);
  ...
  /* copy the hash values of siblings along the path to the root for i-th leaf
   * (index is N+i) */
  j = 0;
...
uint32_t verify(const unsigned char *mu1, const unsigned char *mu2,
  ...
  falcon_verify_res = falcon_verify(
      sig, sig_len, FALCON_SIG_COMPRESSED, pk, FALCON_PUBKEY_SIZE(FALCON_LOGN),
      mu2, MU_LENGTH, buf, FALCON_TMPSIZE_VERIFY(FALCON_LOGN));
  ...
  i_cur = i_in;
  for (i = 0; i < LOGN; i++) {
    /* if i-th LSB of i_in is 1, then for i-th node on the path to the root, its
     * parent has hash value H(AP || x), where x is the hash value of this node
     * and AP is some hash value from the AuthPath i.e. this node's sibling */
    if (i_cur & 1) {
...
int main() {
  ...
  for (benchmark_iteration = 0; benchmark_iteration < BENCHMARK_ITERATION;
    ...
    cycle4 = cpucycles();
    verify_res =
        verify(mu1, mu2, i_in, j_in, v, y, ap, pk, sig, sig_len, tree + 1);
    cycle5 = cpucycles();

    printf("%lld,%lld,%lld,%lld,%lld,%u\n", cycle4 - cycle3,
           eval_falcon_keygen_cycle, eval_falcon_sign_cycle, cycle5 - cycle4,
           verify_falcon_cycle, verify_res);
  }
